...
import { createAzureOpenAIClient, getAzureOpenAIModel } from '@/lib/azure-openai-config'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      modelName = getAzureOpenAIModel()
    } catch (configError) {
      const errorMessage = configError instanceof Error ? configError.message : 'Azure OpenAI configuration error'
      return NextResponse.json({ 
        error: 'Azure OpenAI configuration failed', 
    ...
    const stream = createUIMessageStream({
      ...
      execute: async ({ writer }) => {
        try {
          ...
          // Stream the text generation using Azure OpenAI
